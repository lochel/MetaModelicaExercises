package Assignment "Assignment.mo"
  ...
  algorithm 
    (outEnv, outInteger) := matchcontinue(inEnv, inExp)
      ...
      case (env1, UNARY(unOp=unop, exp=e)) equation 
        (env2, v1) = eval(env1, e);
  ...
  algorithm 
    outInteger := match(inProgram)
      ...
      case (PROGRAM(expLst = assignments,exp = exp)) equation 
