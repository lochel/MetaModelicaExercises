package SymbolicDerivative
  ...
  function diff "Symbolic differentation, e.g. d/dx x^2 = 2*x is performed by
    calling diff(x^2,x) => 2*x"
    input Exp expr;
  ...
  function simplifyExp "When differentating an expression, you often end up 
    with lots of expressions that you can simplify (e.g. 1*x = x).
  ...
  algorithm
    simpleExpr := matchcontinue(expr)
      ...
      // Simplify addition
      case ADD(INT(i1), INT(i2)) equation
        i = i1 + i2;
      then INT(i);
      ...
      case MUL(INT(i1), INT(i2)) equation
        i = i1 * i2;
      then INT(i);
  ...
  function expStr "Translates an Exp into a String"
    input Exp exp;
    output String str;
  algorithm
    str := matchcontinue(exp)
      ...
      case CALL("der", {e}) equation
        res = expStr(e);
      then "(" + res + ")'";
        
      case CALL("pow", {e, INT(i)}) equation
        res = expStr(e);
      then res + "^" + intString(i);
        
      case CALL(id, expList) equation
        res = expListStr(expList);
      then id + "(" + res + ")";
      
      else then "#UNKNOWN_EXP#";
    end matchcontinue;
  end expStr;

  function expListStr "Translates a list of Exp into a comma-separated String"
  ...
  function binExpStr "Translates a binary expression (lhs op rhs) into a String"
    input Exp lhs;
    input String op;
    input Exp rhs;
    output String str;
